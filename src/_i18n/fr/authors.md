...
**Contributions interactives et multimédias.** Une pièce dont le contenu est produit par JavaScript doit être accompagnée d'une version linéaire en texte du même matériel, fournie dans un document distinct. La pièce interactive demeure l'œuvre d'auteur ; la version texte est ce qui la rend citable, trouvable par les moteurs de recherche et accessible aux lecteurs d'écran. Sans elle, une pièce produite en JavaScript n'expose presque aucun texte à qui utilise une technologie d'assistance ou cherche à la trouver.
