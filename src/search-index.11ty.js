...
class SearchIndex {
  ...
  render({ collections, issues }) {
    ...
    // Interactives are no longer indexed from issues.js. Each one now has a
    // companion article carrying its full linear text, which reaches this index
    // through collections.allIssueArticles like any other page -- with real body
    // text rather than the ~47 characters of markup the raw interactive exposes.
    // Indexing both would return two results for one work.
